...
static struct lfs_attr* const lfs_file_find_attribute(
        lfs_t *lfs,lfs_file_t* file, uint8_t type)
{
    for (unsigned i = 0; i < file->cfg->attr_count; i++) {
...
static lfs_ssize_t lfs_file_rawgetattr(lfs_t *lfs, lfs_file_t* file,
    ...
    struct lfs_attr* const attr = lfs_file_find_attribute(lfs, file, type);
    if(attr != NULL) {
...
static int lfs_file_rawremoveattr(lfs_t *lfs, lfs_file_t* file, uint8_t type) {
    ...
    if(lfs_file_find_attribute(lfs, file, type) != NULL) {
        LFS_ASSERT(false);
